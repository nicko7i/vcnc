...
//      console.log(`maxIndex = ${self.maxIndex}`);
    ...
    if (self.binCount[index - self.minIndex] === undefined) {
      ...
      self.totalBins += 1;
      console.log(`index=${index} minIndex=${self.minIndex} totalBins=${self.totalBins}`);
  ...
  ReleaseBinTest() {
    ...
    if (self.pmReadBins[self.minIndex] !== undefined) {
      self.avgTimeVcncDA += self.pmReadBins[self.minIndex];
...
//    const rts = (Date.now() - currts) / 1000;
  ...
  AvgDaVcnc() {
    ...
    return ((count === 0) ? 0 : (this.avgTimeVcncDA / count) / 1000);
  }
