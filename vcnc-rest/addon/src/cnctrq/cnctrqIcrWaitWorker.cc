...
namespace cnc {
  namespace cnctrq {
    ...
    cnctrqIcrWaitWorker::cnctrqIcrWaitWorker(int trq_id
                                             , int mod
    ...
    //  Destructor.
    //
    cnctrqIcrWaitWorker::~cnctrqIcrWaitWorker() { delete _client; }
    //
    //  Execute
    //
    void cnctrqIcrWaitWorker::Execute() {
      if (_errcode) 
        return;  // An error has occurred before we were called.
      _errcode = _client->IcrWait(_mod);
    }
    //
    //  HandleOKCallback
    //
    void cnctrqIcrWaitWorker::HandleOKCallback() {
      //
